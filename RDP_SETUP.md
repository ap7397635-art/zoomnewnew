...
- Create a **separate** worker in dashboard (`RDP-Box-1`, `RDP-Box-2`, …) — just pick **any number** for `capacity_max` (worker overrides it anyway)
- Each gets its own token → its own `.env`
- Tasks are **automatically distributed** with **capacity-weighted fair share** (worker v6+) — every box reports its OWN live safe-capacity from RAM/CPU; bigger boxes get proportionally more bots.

**Zero-config capacity (worker v7-lean default — `AUTO_CAPACITY=true`):**
Worker computes live safe-capacity every heartbeat from THIS machine's actual RAM + CPU. v7-lean uses heavy Chrome trimming (no images, no GPU, capped V8 heap, throttled renderer) so per-bot footprint drops to ~200 MB / 0.18 CPU.

| RDP specs              | Auto-computed capacity (v7-lean defaults: 200 MB/bot, 5 bots/CPU) |
|------------------------|--------------------------------------------------------------------|
| 8 CPU / 64 GB RAM      | ~40 bots                                                           |
| 4 CPU / 32 GB RAM      | ~20 bots                                                           |
| 4 CPU / 16 GB RAM      | ~20 bots                                                           |
| 2 CPU / 8 GB RAM       | ~10 bots                                                           |
| 2 CPU / 4 GB RAM       | ~10 bots                                                           |

If your RDPs are stable above this and you want MORE bots per box, tune `.env`:

```env
BOTS_PER_CPU=8           # raise from 5 → 64 bots on an 8-CPU box
RAM_PER_BOT_MB=180       # lower from 200 if your Chrome is even leaner
RAM_HEADROOM_PCT=15      # lower from 20 if you have a dedicated RDP
MAX_CAPACITY_HARD_CAP=300 # absolute upper bound the worker will report
```

To force a manual cap and ignore auto-detection:
```env
AUTO_CAPACITY=false      # dashboard uses the capacity_max you set
```

### Distribution mode: GREEDY vs WEIGHTED
v7 dashboard uses **GREEDY fill by default** — whichever RDP polls first grabs bots up to its full capacity, the next polling RDP gets the leftover. Result: one RDP fills completely, then the next, then the next. This is the user-requested behavior so big tasks concentrate on the fastest-responding boxes.

To switch to capacity-weighted spread (each RDP gets bots proportional to its capacity from the start), set on the **backend** `.env`:
```env
DISTRIBUTION_MODE=weighted
```

**Worker v7-lean features (all automatic, no setup needed):**
- `SPAWN_BATCH=0` (default) — auto-tunes parallel Chrome spawns from local CPU + RAM.
- `HEADLESS=false` (default) — windowed Chrome resists Zoom's anti-bot kicks better. Override with `HEADLESS=true` only if your RDP truly has no display.
- `BOT_REJOIN_MAX=2` — if Zoom kicks a bot within the first `KICK_DETECT_WINDOW=180` seconds, the worker auto-rejoins up to 2× before giving up. Stops "members nikalne se".
- `PRE_SPAWN_FREE_RAM_PCT=12` — if free RAM drops below 12 % mid-spawn, the worker pauses for up to 60 s for RAM to recover. Unspawned members get picked up by other RDPs via the 8-second server-side mop-up.
- **Stealth user-agent + CDP override** — `navigator.webdriver` is hidden, real Chrome UA, languages, plugins set so Zoom's bot-detection JS sees a normal browser.
- **Ultra-low RAM/CPU Chrome flags**:
  - `--js-flags=--max-old-space-size=256` — caps V8 heap to 256 MB (default is 4 GB!)
  - `--disable-gpu`, `--disable-webgl`, `--disable-3d-apis`, `--disable-accelerated-*`
  - `--blink-settings=imagesEnabled=false` + `profile.managed_default_content_settings.images=2` — block all images
  - `--window-size=800,600` + `--force-device-scale-factor=0.75` — smaller render surface
  - `--media-cache-size=1`, `--disk-cache-size=1`, `--aggressive-cache-discard` — kill caches
  - `--renderer-process-limit=2` — limit Chrome's own renderer fan-out
  - `--disable-features=Translate,BackForwardCache,OptimizationHints,MediaRouter,GlobalMediaControls,...`
- **Post-join CPU savings** (run inside the bot's tab once joined):
  - Hide all `<video>` / `<canvas>` elements via injected CSS — drops remote video render
  - Pause/null all video sources every 4 s (recovers from speaker-switch DOM rebuilds)
  - Force `document.visibilityState = 'hidden'` — Chrome throttles the renderer ~10× when tab is "hidden"
- **Live RAM/CPU display on dashboard** — `Workers` page shows `current/effective_cap (auto, set: manual_cap)` plus free-GB and core count.

**Tuning for 30+ RDPs:**
- Set `POLL_INTERVAL=5` (default in v7) so newly-created tasks get picked up in ≤5 seconds.
- Mop-up grace stays at **8 s** server-side — if a worker crashes mid-task, the next polling worker claims the unclaimed bots within 8 s (greedy mode picks them up automatically as part of the normal first-polled-takes-all flow).
- You DON'T need to manually set `capacity_max` per RDP anymore — `AUTO_CAPACITY=true` is the default.
